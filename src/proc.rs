...
impl<'d> Processor<'d> {
    ...
    // Assert
    fn _require(&self, custom_reason: Option<&'static str>) -> InsRes<()> {
        let m = self.m.unwrap();
        if m.count > 0 {
            Ok(())
        } else {
            match m.reason {
                RequireReason::Custom => Err(InsErr::ExpectedNotFound(custom_reason.unwrap())),
                RequireReason::ExpectedNotChar(c) => Err(InsErr::ExpectedCharNotFound { expected: c, got: m.char.unwrap() }),
                RequireReason::ExpectedChar(c) => Err(InsErr::UnexpectedCharFound(c)),
                RequireReason::ExpectedMatch(m) => Err(InsErr::ExpectedMatchNotFound(m)),
            }
        }
    }
    pub fn require(&self) -> InsRes<()> {
        self._require(None)
    }
    pub fn require_with_reason(&self, reason: &'static str) -> InsRes<()> {
    ...
    pub fn peek_offset(&self, offset: usize) -> InsRes<u8> {
        self.maybe_read(offset).ok_or(InsErr::UnexpectedEnd)
    }
    pub fn peek_eof(&self) -> Option<u8> {
        self.maybe_read(0)
    }
    pub fn peek(&self) -> InsRes<u8> {
        self.maybe_read(0).ok_or(InsErr::UnexpectedEnd)
    }

    /// Skip the next `count` characters (can be zero).
    /// Will result in an error if exceeds bounds.
    pub fn skip_amount(&mut self, count: usize) -> InsRes<()> {
        // Check for zero to prevent underflow as type is usize.
        if count == 0 || self.in_bounds(count - 1) {
            self.consume(count);
            Ok(())
        } else {
            Err(InsErr::UnexpectedEnd)
        }
    }
    /// Skip and return the next character.
    /// Will result in an error if exceeds bounds.
    pub fn skip(&mut self) -> InsRes<u8> {
        if !self.at_end() {
            let c = self.read(0);
            self.consume(1);
            Ok(c)
    ...
    pub fn accept(&mut self) -> InsRes<u8> {
        if !self.at_end() {
            let c = self.read(0);
            self.shift(1);
            Ok(c)
    ...
    pub fn accept_amount(&mut self, count: usize) -> InsRes<()> {
        // Check for zero to prevent underflow as type is usize.
        if count == 0 || self.in_bounds(count - 1) {
            self.shift(count);
            Ok(())
