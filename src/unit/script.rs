...
fn parse_comment_single<'d, 'p>(proc: &'p mut Processor<'d>) -> InsRes<()> {
...
fn parse_comment_multi<'d, 'p>(proc: &'p mut Processor<'d>) -> InsRes<()> {
    cascade_return!(proc.match_seq(b"/*").expect().keep());
...
fn parse_string<'d, 'p>(proc: &'p mut Processor<'d>) -> InsRes<()> {
    ...
    loop {
        ...
        if cascade_return!(proc.match_line_terminator().keep().matched()) {
            if !escaping {
                return Err(InsErr::ExpectedNotFound("Unterminated JavaScript string"));
            }
        }
...
fn parse_template<'d, 'p>(proc: &'p mut Processor<'d>) -> InsRes<()> {
...
pub fn process_script<'d, 'p>(proc: &'p mut Processor<'d>) -> InsRes<()> {
